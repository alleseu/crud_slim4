...
class MantenedorController extends BaseController {
	...
	public function obtenerProductos($request, $response, $args) {
		...
		$data = $this->container->get('servicio_producto')->obtenerTodo();
	...
	public function obtenerCategorias($request, $response, $args) {
		...
		$data = $this->container->get('servicio_categoria')->obtenerTodo();
	...
	public function obtenerProducto($request, $response, $args) {
		...
		if (ctype_digit($id)) {
			...
			$data = $this->container->get('servicio_producto')->obtener($id);
		...
		if (!empty($codigo) && !empty($nombre) && !empty($categoria)) {
			...
			if (ctype_digit($codigo) && ctype_digit($categoria)) {
				...
				if (strlen($codigo) == 3 && strlen($nombre) <= 30) {
					...
					$busqueda = $this->container->get('servicio_categoria')->buscarId($categoria);

					if ($busqueda > 0) {

						//LLAMA A LA FUNCIÓN QUE BUSCA SI ESTÁ REGISTRADO EL CÓDIGO DEL PRODUCTO. (RETORNA LA CANTIDAD DE REGISTROS).
						$busqueda = $this->container->get('servicio_producto')->buscarCodigo($codigo);

						if ($busqueda == 0) {

							//LLAMA A LA FUNCIÓN PARA INSERTAR UN PRODUCTO NUEVO.
							$this->container->get('servicio_producto')->insertar($codigo, $nombre, $categoria);
			...
			if (ctype_digit($id) && ctype_digit($codigo) && ctype_digit($categoria)) {
				...
				if (strlen($codigo) == 3 && strlen($nombre) <= 30) {
					...
					if ($busqueda > 0) {
						...
						$busqueda = $this->container->get('servicio_producto')->buscarCodigoFiltrado($id, $codigo);

						if ($busqueda == 0) {

							//LLAMA A LA FUNCIÓN PARA ACTUALIZAR UN PRODUCTO.
							$this->container->get('servicio_producto')->actualizar($id, $codigo, $nombre, $categoria);
	...
	public function eliminarProducto($request, $response, $args) {
		...
		if (ctype_digit($id)) {
			...
			$busqueda = $this->container->get('servicio_producto')->buscarId($id);

			if ($busqueda > 0) {

				//LLAMA A LA FUNCIÓN PARA ELIMINAR UN PRODUCTO.
				$this->container->get('servicio_producto')->eliminar($id);
